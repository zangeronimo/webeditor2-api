using System.Linq.Expressions;
using Microsoft.EntityFrameworkCore;
using Webeditor.Domain.Entities.Recipes;
using Webeditor.Domain.Interfaces.Recipes;
using Webeditor.Domain.Models;
using Webeditor.Domain.Models.Recipes;
using Webeditor.Infra.Context;

namespace Webeditor.Infra.Repositories.Recipes;

public class RecipeRateRepository : BaseRepository<RecipeRate>, IRecipeRateRepository
{
  public RecipeRateRepository(AppDbContext context) : base(context)
  { }

  public async Task<PaginationResultModel<RecipeRate>> GetAllAsync(long systemCompanyId, RecipeRateFilterModel filter, BasePaginationModel pagination)
  {
    try
    {
      var query = DbSet.AsQueryable();

      if (filter?.Guid != null)
        query = query.Where(rate => rate.Guid == filter.Guid);

      if (filter?.Active != null)
        query = query.Where(rate => rate.Active == filter.Active);

      if (filter?.InitialDate != null)
        query = query.Where(rate => rate.CreatedAt >= filter.InitialDate);

      if (filter?.FinalDate != null)
        query = query.Where(rate => rate.CreatedAt <= filter.FinalDate);

      if (filter?.Asc == true)
        query = query.OrderBy(Ordenation(filter?.OrderBy));
      else
        query = query.OrderByDescending(Ordenation(filter?.OrderBy));


      query = query.Where(rate => rate.RemovedAt == null &&
        rate.SystemCompanyId == systemCompanyId);

      var Page = pagination?.Page ?? 0;
      var Items = pagination?.ItemsPerPage ?? 20;

      var Total = query.Count();
      query = query.Skip(Page * Items);
      query = query.Take(Items);

      var result = await query.ToListAsync();
      return new PaginationResultModel<RecipeRate>()
      {
        Result = await query.ToListAsync(),
        Page = Page,
        ItemsPerPage = Items,
        Total = Total
      };
    }
    catch
    {
      throw;
    }
  }

  private Expression<Func<RecipeRate, Object>> Ordenation(string? order)
  {
    if (!string.IsNullOrEmpty(order))
    {
      switch (order.ToLowerInvariant())
      {
        case "active":
          return x => x.Active;
      }
    }

    return x => x.CreatedAt;
  }

  public override async Task<RecipeRate?> GetByGuidAsync(Guid guid, long systemCompanyId)
  {
    try
    {
      return await DbSet.Where(rate => rate.RemovedAt == null &&
        rate.SystemCompanyId == systemCompanyId && rate.Guid == guid)
        .FirstOrDefaultAsync();
    }
    catch
    {
      throw;
    }
  }
}
